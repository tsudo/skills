...
- **The parent issue or project description**, where one states an end state rather than a task.
...
Verbatim matters on the third path. A paraphrase at the source compounds through everything downstream.
...
If you can only see part of the picture, say so and stop. An assessment built on half the work is worse than none, because it looks the same as a good one.
...
Apply only `Verified` hygiene fixes without asking. Every other change - creating work, changing a priority, fixing a relation, changing a state - stops for explicit approval. This skill never mutates a tracker on its own authority.
